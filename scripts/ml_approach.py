...
def extract_color_layout(image, grid_size=4):
...
def extract_features(image):
    ...
    layout_features = extract_color_layout(image)
...
def build_feature_database(dataset, n_neighbors=5, val_size=0.2, random_state=42):
    ...
    nearest_neighbors = NearestNeighbors(n_neighbors=min(n_neighbors, len(features_list)), metric='cosine')
    nearest_neighbors.fit(X_pca)
    
    # Store PCA-transformed features
    features_df['features_pca'] = list(X_pca)
...
def identify_painting(image, model):
    ...
    try:
        ...
        features_pca = model['pca_model'].transform(features_scaled)
        
        # Find nearest neighbors
        distances, indices = model['nearest_neighbors'].kneighbors(features_pca)
        ...
        for i, (dist, idx) in enumerate(zip(distances[0], indices[0])):
            ...
            match_info = model['features_df'].iloc[idx].copy()
...
def save_model(model, model_path='./models/painting_identifier_ml.pkl'):
...
def load_model(model_path='./models/painting_identifier_ml.pkl'):
...
def evaluate_top_n_accuracy(test_dataset, model, n=5):
    """
    Evaluate the model using Top-N accuracy.
    
    Args:
        test_dataset: Dataset of test paintings
        model: Model dictionary with all components
    ...
    for example in tqdm(test_dataset):
        try:
            ...
            # Get predictions
            matches = identify_painting(example['image'], model)
...
def evaluate_mrr(test_dataset, model):
    """
...
def evaluate_test_folder(model, test_folder, dataset, k=5):
    """
    ...
    for filename in tqdm(sorted(os.listdir(test_folder))):
        ...
        try:
            ...
            # Predict
            matches = identify_painting(test_image, model)
            if not matches:
...
def evaluate_ml_approach():
    ...
    model_path = './models/painting_identifier_ml.pkl'
    if os.path.exists(model_path):
        ...
        try:
            ...
            model = build_feature_database(train_ds)
    ...
    val_top1_acc = evaluate_top_n_accuracy(val_ds, model, n=1)
    val_top5_acc = evaluate_top_n_accuracy(val_ds, model, n=5)
    val_mrr = evaluate_mrr(val_ds, model)
    ...
    if os.path.exists(test_folder):
        print("\n=== TEST SET EVALUATION ===")
        test_metrics = evaluate_test_folder(model, test_folder, ds, k=5)
    else:
