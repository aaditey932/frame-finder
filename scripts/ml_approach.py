...
class PaintingTitlePredictor:
    def __init__(self, n_neighbors=5):
        """
        Initialize the Painting Title Predictor using traditional ML.
    ...
    def build_feature_database(self, dataset, test_size=0.2, random_state=42):
...
def evaluate_from_folder(identifier, test_folder, dataset, k=5):
    """
    Evaluate model performance on test images from a folder.
    
    Args:
        identifier: Trained PaintingTitlePredictor instance
    ...
    print(f"Top-{k} Accuracy: {correct_at_k / total:.2%}")
...
def main():
    ...
    try:
        ...
        ds = ds.select(range(100))  # Using more samples for better evaluation
    except Exception as e:
    ...
    identifier = PaintingTitlePredictor(n_neighbors=5)
    ...
    top1_acc = identifier.evaluate_top_n_accuracy(test_ds, n=1)  # Top-1 accuracy
    top5_acc = identifier.evaluate_top_n_accuracy(test_ds, n=5)  # Top-5 accuracy
    mrr = identifier.evaluate_mrr(test_ds)  # Mean Reciprocal Rank
    
    # Evaluate on folder of test images if it exists
    test_folder = "data/raw/testing_images"
    if os.path.exists(test_folder):
        print("\n=== FOLDER EVALUATION ===")
        evaluate_from_folder(identifier, test_folder, ds, k=5)
    else:
        print(f"\nTest folder {test_folder} not found. Skipping folder evaluation.")
